const CalcEquation = () => {
  ...
  function showResult() {
    ...
    console.log(clc.blue("Result:", result));

    fs.writeFile(resultFilePath, result, () => {});

    rl.on("close", () => {
      console.log(clc.yellow(`Результат добавлен в журнал: ${resultFilePath}`));
    });
    rl.close();
  }
  ...
  function calculateResult(a, b, c) {
    if (!a.trim() || !b.trim() || !c.trim()) {
      console.log(clc.red("Вы не ввели значение коэффициента"));
      process.exit(1);
    } else {
      Number(a, b, c);
      const D = b * b - 4 * a * c;
      ...
      if (isNaN(a) || isNaN(b) || isNaN(c)) {
        console.log(
          clc.red("Неправильный ввод коэффициента. Введите три числа.")
        );
        process.exit(2);
      }

      if (a == 0) {
        console.log(clc.red("Ошибка! коэффициент 'а' не может равняться 0."));
        process.exit(3);
      }
